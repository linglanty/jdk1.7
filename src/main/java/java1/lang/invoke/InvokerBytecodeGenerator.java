...
class InvokerBytecodeGenerator {
    ...
    private final static java.util.HashMap<String,Integer> DUMP_CLASS_FILES_COUNTERS;
    /** debugging flag for saving generated class files */
    private final static File DUMP_CLASS_FILES_DIR;

    static {
        if (DUMP_CLASS_FILES) {
            DUMP_CLASS_FILES_COUNTERS = new java.util.HashMap<>();
            try {
    ...
    java.util.Map<Object, CpPatch> cpPatches = new java.util.HashMap<>();
    ...
    Object[] cpPatches(byte[] classFile) {
        ...
        for (CpPatch p : cpPatches.values()) {
            if (p.index >= size)
                throw new InternalError("in cpool["+size+"]: "+p+"\n"+ java.util.Arrays.toString(java.util.Arrays.copyOf(classFile, 20)));
            res[p.index] = p.value;
        }
        return res;
    }
