...
assertEquals("[A, B, C]", (String) caToString2.invokeExact('A', "BC".toCharArray()));
    ...
    private void asSpreaderChecks(Class<?> arrayType, int arrayLength) {
        ...
        if (arrayType != Object[].class && arrayLength != 0) {
            ...
            if (sawProblem) {
                java.util.ArrayList<Class<?>> ptypes = new java.util.ArrayList<>(type().parameterList());
                for (int i = nargs - arrayLength; i < nargs; i++) {
...
assertEquals("[three, thee, tee]", asListFix.invoke((Object)argv).toString());
    ...
    static LambdaForm reinvokerForm(MethodType mtype) {
        ...
        Object[] targetArgs = java.util.Arrays.copyOfRange(names, THIS_BMH, ARG_LIMIT, Object[].class);
