 ...
 * {@link java.util.concurrent.ForkJoinTask}s.
...
public class ForkJoinWorkerThread extends Thread {
    ...
    protected void onStart() {
        queue = new java.util.concurrent.ForkJoinTask<?>[INITIAL_QUEUE_CAPACITY];
     ...
     */
    private static final boolean casSlotNull(java.util.concurrent.ForkJoinTask<?>[] q, int i,
     ...
     */
    private static final void writeSlot(java.util.concurrent.ForkJoinTask<?>[] q, int i,
                                        java.util.concurrent.ForkJoinTask<?> t) {
     ...
     */
    final void pushTask(java.util.concurrent.ForkJoinTask<?> t) {
    ...
    private void growQueue() {
        java.util.concurrent.ForkJoinTask<?>[] oldQ = queue;
        int size = oldQ != null ? oldQ.length << 1 : INITIAL_QUEUE_CAPACITY;
        if (size > MAXIMUM_QUEUE_CAPACITY)
            throw new java.util.concurrent.RejectedExecutionException("Queue capacity exceeded");
        if (size < INITIAL_QUEUE_CAPACITY)
            size = INITIAL_QUEUE_CAPACITY;
        java.util.concurrent.ForkJoinTask<?>[] q = queue = new java.util.concurrent.ForkJoinTask<?>[size];
        int mask = size - 1;
        int top = queueTop;
        int oldMask;
    ...
    final java.util.concurrent.ForkJoinTask<?> locallyDeqTask() {
        java.util.concurrent.ForkJoinTask<?> t; int m, b, i;
        java.util.concurrent.ForkJoinTask<?>[] q = queue;
    ...
    private java.util.concurrent.ForkJoinTask<?> popTask() {
        ...
        if (q != null && (m = q.length - 1) >= 0) {
            for (int s; (s = queueTop) != queueBase;) {
                ...
                java.util.concurrent.ForkJoinTask<?> t = q[i];
     ...
     * @param t the task. Caller must ensure non-null.
     */
    final boolean unpushTask(java.util.concurrent.ForkJoinTask<?> t) {
     ...
     */
    final void execTask(java.util.concurrent.ForkJoinTask<?> t) {
        currentSteal = t;
        for (;;) {
            if (t != null)
                t.doExec();
    ...
    final void cancelTasks() {
        java.util.concurrent.ForkJoinTask<?> cj = currentJoin; // try to cancel ongoing tasks
        if (cj != null && cj.status >= 0)
            cj.cancelIgnoringExceptions();
        java.util.concurrent.ForkJoinTask<?> cs = currentSteal;
        if (cs != null && cs.status >= 0)
            cs.cancelIgnoringExceptions();
        while (queueBase != queueTop) {
            java.util.concurrent.ForkJoinTask<?> t = deqTask();
            if (t != null)
     ...
     */
    final int drainTasksTo(Collection<? super java.util.concurrent.ForkJoinTask<?>> c) {
        int n = 0;
        while (queueBase != queueTop) {
            java.util.concurrent.ForkJoinTask<?> t = deqTask();
            if (t != null) {
    ...
    final java.util.concurrent.ForkJoinTask<?> pollTask() {
        ...
        java.util.concurrent.ForkJoinTask<?> t = pollLocalTask();
     ...
     */
    final int joinTask(java.util.concurrent.ForkJoinTask<?> joinMe) {
        java.util.concurrent.ForkJoinTask<?> prevJoin = currentJoin;
        currentJoin = joinMe;
     ...
     */
    private boolean localHelpJoinTask(java.util.concurrent.ForkJoinTask<?> joinMe) {
        int s, i; java.util.concurrent.ForkJoinTask<?>[] q; java.util.concurrent.ForkJoinTask<?> t;
     ...
     */
    private boolean helpJoinTask(java.util.concurrent.ForkJoinTask<?> joinMe) {
        boolean helped = false;
        int m = pool.scanGuard & SMASK;
        ForkJoinWorkerThread[] ws = pool.workers;
        if (ws != null && ws.length > m && joinMe.status >= 0) {
            int levels = MAX_HELP;              // remaining chain length
            java.util.concurrent.ForkJoinTask<?> task = joinMe;      // base of chain
            outer:for (ForkJoinWorkerThread thread = this;;) {
                ...
                for (;;) {
                    java.util.concurrent.ForkJoinTask<?>[] q; int b, i;
                    if (joinMe.status < 0)
                        break outer;
                    ...
                    if (t != null && v.queueBase == b &&
                        ...
                        java.util.concurrent.ForkJoinTask<?> ps = currentSteal;
                        currentSteal = t;
                        t.doExec();
                        currentSteal = ps;
                        helped = true;
                ...
                java.util.concurrent.ForkJoinTask<?> next = v.currentJoin;
     ...
     * @return t's status
     */
    private int tryDeqAndExec(java.util.concurrent.ForkJoinTask<?> t) {
        int m = pool.scanGuard & SMASK;
        ForkJoinWorkerThread[] ws = pool.workers;
    ...
    final void helpQuiescePool() {
        boolean active = true;
        java.util.concurrent.ForkJoinTask<?> ps = currentSteal; // to restore below
        ForkJoinPool p = pool;
