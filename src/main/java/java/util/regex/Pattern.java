 ...
 * a {@link java.util.regex.Matcher} object that can match arbitrary {@link
 ...
 *    by the {@link java.util.regex.Matcher} class: Repeated invocations of the {@link
 *    java.util.regex.Matcher#find find} method will resume where the last match left off,
    ...
    implements java.io.Serializable
{
    ...
    public java.util.regex.Matcher matcher(CharSequence input) {
        ...
        java.util.regex.Matcher m = new java.util.regex.Matcher(this, input);
        return m;
    }
    ...
    public String[] split(CharSequence input, int limit) {
        ...
        java.util.regex.Matcher m = matcher(input);
...
loop:   for(int x=0, offset=0; x<nCodePoints; x++, offset+=len) {
    ...
    private void RemoveQEQuoting() {
        ...
        while (i < pLen) {
            int c = temp[i++];
            if (! java.util.regex.ASCII.isAscii(c) || java.util.regex.ASCII.isAlnum(c)) {
                newtemp[j++] = c;
            } else if (c != '\\') {
    ...
    private int peekPastWhitespace(int ch) {
        while (java.util.regex.ASCII.isSpace(ch) || ch == '#') {
            while (java.util.regex.ASCII.isSpace(ch))
                ch = temp[++cursor];
            if (ch == '#') {
                ch = peekPastLine();
            }
    ...
    private int escape(boolean inclass, boolean create) {
        ...
        case 'D':
            if (create) root = has(UNICODE_CHARACTER_CLASS)
                               ? new Utype(java.util.regex.UnicodeProp.DIGIT).complement()
                               : new Ctype(java.util.regex.ASCII.DIGIT).complement();
            return -1;
        case 'E':
        case 'F':
            break;
        ...
        case 'S':
            if (create) root = has(UNICODE_CHARACTER_CLASS)
                               ? new Utype(java.util.regex.UnicodeProp.WHITE_SPACE).complement()
                               : new Ctype(java.util.regex.ASCII.SPACE).complement();
            return -1;
        case 'T':
        case 'U':
        case 'V':
        ...
        case 'W':
            if (create) root = has(UNICODE_CHARACTER_CLASS)
                               ? new Utype(java.util.regex.UnicodeProp.WORD).complement()
                               : new Ctype(java.util.regex.ASCII.WORD).complement();
            return -1;
        case 'X':
        case 'Y':
            break;
        ...
        case 'd':
            if (create) root = has(UNICODE_CHARACTER_CLASS)
                               ? new Utype(java.util.regex.UnicodeProp.DIGIT)
                               : new Ctype(java.util.regex.ASCII.DIGIT);
            return -1;
        case 'e':
            return '\033';
        case 'f':
        ...
        case 's':
            if (create) root = has(UNICODE_CHARACTER_CLASS)
                               ? new Utype(java.util.regex.UnicodeProp.WHITE_SPACE)
                               : new Ctype(java.util.regex.ASCII.SPACE);
            return -1;
        case 't':
            return '\t';
        case 'u':
        ...
        case 'w':
            if (create) root = has(UNICODE_CHARACTER_CLASS)
                               ? new Utype(java.util.regex.UnicodeProp.WORD)
                               : new Ctype(java.util.regex.ASCII.WORD);
            return -1;
        case 'x':
            return x();
        case 'y':
                                ...
                                boolean maybeComplement)
    {
        ...
        if (i != -1) {
            ...
            if (name.startsWith("In")) {
                ...
                java.util.regex.UnicodeProp uprop = java.util.regex.UnicodeProp.forName(name);
                if (uprop != null)
                ...
                if (has(UNICODE_CHARACTER_CLASS)) {
                    java.util.regex.UnicodeProp uprop = java.util.regex.UnicodeProp.forPOSIXName(name);
                    if (uprop != null)
                        node = new Utype(uprop);
                }
                if (node == null)
    ...
    private String groupname(int ch) {
        ...
        while (java.util.regex.ASCII.isLower(ch=read()) || java.util.regex.ASCII.isUpper(ch) ||
               java.util.regex.ASCII.isDigit(ch)) {
            sb.append(Character.toChars(ch));
        }
        if (sb.length() == 0)
    ...
    private Node group0() {
        ...
        if (ch == '?') {
            ...
            case '<':   // (?<xxx)  look behind
                ch = read();
                if (java.util.regex.ASCII.isLower(ch) || java.util.regex.ASCII.isUpper(ch)) {
    ...
    private Node closure(Node prev) {
        ...
        case '{':
            ch = temp[cursor+1];
            if (java.util.regex.ASCII.isDigit(ch)) {
                skip();
                int cmin = 0;
                do {
                    cmin = cmin * 10 + (ch - '0');
                } while (java.util.regex.ASCII.isDigit(ch = read()));
                int cmax = cmin;
                if (ch == ',') {
                    ch = read();
                    cmax = MAX_REPS;
                    if (ch != '}') {
                        cmax = 0;
                        while (java.util.regex.ASCII.isDigit(ch)) {
                            cmax = cmax * 10 + (ch - '0');
                            ch = read();
                        }
                    }
    ...
    private int x() {
        int n = read();
        if (java.util.regex.ASCII.isHexDigit(n)) {
            int m = read();
            if (java.util.regex.ASCII.isHexDigit(m)) {
                return java.util.regex.ASCII.toDigit(n) * 16 + java.util.regex.ASCII.toDigit(m);
            }
        } else if (n == '{' && java.util.regex.ASCII.isHexDigit(peek())) {
            int ch = 0;
            while (java.util.regex.ASCII.isHexDigit(n = read())) {
                ch = (ch << 4) + java.util.regex.ASCII.toDigit(n);
    ...
    private int uxxxx() {
        ...
        for (int i = 0; i < 4; i++) {
            int ch = read();
            if (!java.util.regex.ASCII.isHexDigit(ch)) {
                throw error("Illegal Unicode escape sequence");
            }
            n = n * 16 + java.util.regex.ASCII.toDigit(ch);
        }
        return n;
    }
    ...
    private static final class BitClass extends BmpCharProperty {
        ...
        BitClass add(int c, int flags) {
            ...
            if ((flags & CASE_INSENSITIVE) != 0) {
                if (java.util.regex.ASCII.isAscii(c)) {
                    bits[java.util.regex.ASCII.toUpper(c)] = true;
                    bits[java.util.regex.ASCII.toLower(c)] = true;
    ...
    private CharProperty newSingle(final int ch) {
        if (has(CASE_INSENSITIVE)) {
            ...
            } else if (java.util.regex.ASCII.isAscii(ch)) {
                lower = java.util.regex.ASCII.toLower(ch);
                upper = java.util.regex.ASCII.toUpper(ch);
                if (lower != upper)
    ...
    private Node newSlice(int[] buf, int count, boolean hasSupplementary) {
        ...
        if (has(CASE_INSENSITIVE)) {
            ...
            for (int i = 0; i < count; i++) {
                tmp[i] = java.util.regex.ASCII.toLower(buf[i]);
            }
    ...
    static class Node extends Object {
         ...
         */
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            matcher.last = i;
    ...
    static class LastNode extends Node {
         ...
         */
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            if (matcher.acceptMode == java.util.regex.Matcher.ENDANCHOR && i != matcher.to)
                return false;
            matcher.last = i;
            matcher.groups[0] = matcher.first;
            matcher.groups[1] = matcher.last;
    ...
    static final class Utype extends CharProperty {
        final java.util.regex.UnicodeProp uprop;
        Utype(java.util.regex.UnicodeProp uprop) { this.uprop = uprop; }
        boolean isSatisfiedBy(int ch) {
            return uprop.is(ch);
        }
    }
    ...
    static final class Ctype extends BmpCharProperty {
        ...
        boolean isSatisfiedBy(int ch) {
            return ch < 128 && java.util.regex.ASCII.isType(ch, ctype);
        }
    }
    ...
    static class SliceI extends SliceNode {
        ...
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            ...
            for (int j=0; j<len; j++) {
                ...
                if (buf[j] != c &&
                    buf[j] != java.util.regex.ASCII.toLower(c))
                    return false;
            }
            return next.match(matcher, i+len, seq);
        }
    ...
    static class SliceIS extends SliceNode {
        ...
        int toLower(int c) {
            return java.util.regex.ASCII.toLower(c);
        }
        boolean match(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            int[] buf = buffer;
            int x = i;
    ...
    private CharProperty caseInsensitiveRangeFor(final int lower,
        ...
        return new CharProperty() {
            boolean isSatisfiedBy(int ch) {
                return inRange(lower, ch, upper) ||
                    java.util.regex.ASCII.isAscii(ch) &&
                        (inRange(lower, java.util.regex.ASCII.toUpper(ch), upper) ||
                         inRange(lower, java.util.regex.ASCII.toLower(ch), upper));
            }};
    }
    ...
    static final class Curly extends Node {
        ...
        boolean match0(java.util.regex.Matcher matcher, int i, int j, CharSequence seq) {
            if (j >= cmax) {
        ...
        // i is the index to start matching at
        // j is the number of atoms that have matched
        boolean match1(java.util.regex.Matcher matcher, int i, int j, CharSequence seq) {
            for (;;) {
        ...
        boolean match2(java.util.regex.Matcher matcher, int i, int j, CharSequence seq) {
            for (; j < cmax; j++) {
                if (!atom.match(matcher, i, seq))
                    break;
                if (i == matcher.last)
    ...
    static final class GroupHead extends Node {
        ...
        boolean matchRef(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            int save = matcher.locals[localIndex];
    ...
    static class Loop extends Node {
        ...
        boolean matchInit(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            ...
            for (int index=0; index<groupSize; index++) {
                ...
                if (c1 != c2) {
                    if (doUnicodeCase) {
                        ...
                        if (java.util.regex.ASCII.toLower(c1) != java.util.regex.ASCII.toLower(c2))
                            return false;
                    }
                }
    ...
    static final class Bound extends Node {
        ...
        boolean isWord(int ch) {
            return useUWORD ? java.util.regex.UnicodeProp.WORD.is(ch)
                            : (ch == '_' || Character.isLetterOrDigit(ch));
        }

        int check(java.util.regex.Matcher matcher, int i, CharSequence seq) {
            int ch;
            boolean left = false;
            int startIndex = matcher.from;
            int endIndex = matcher.to;
     ...
     */
    private static boolean hasBaseCharacter(java.util.regex.Matcher matcher, int i,
                                            CharSequence seq)
    {
...
///////////////////////////////////////////////////////////////////////////////
    ...
    private static class CharPropertyNames {
        ...
        static {
            ...
            defCtype("Alnum", java.util.regex.ASCII.ALNUM);  // Alphanumeric characters
            defCtype("Alpha", java.util.regex.ASCII.ALPHA);  // Alphabetic characters
            defCtype("Blank", java.util.regex.ASCII.BLANK);  // Space and tab characters
            defCtype("Cntrl", java.util.regex.ASCII.CNTRL);  // Control characters
            defRange("Digit", '0', '9');     // Numeric characters
            defCtype("Graph", java.util.regex.ASCII.GRAPH);  // printable and visible
            defRange("Lower", 'a', 'z');     // Lower-case alphabetic
            defRange("Print", 0x20, 0x7E);   // Printable characters
            defCtype("Punct", java.util.regex.ASCII.PUNCT);  // Punctuation characters
            defCtype("Space", java.util.regex.ASCII.SPACE);  // Space characters
            defRange("Upper", 'A', 'Z');     // Upper-case alphabetic
            defCtype("XDigit", java.util.regex.ASCII.XDIGIT); // hexadecimal digits
