...
public class Timer {
     ...
     * @throws NullPointerException if {@code task} is null
     */
    private void sched(java.util.TimerTask task, long time, long period) {
        if (time < 0)
        ...
        synchronized(queue) {
            ...
            synchronized(task.lock) {
                if (task.state != java.util.TimerTask.VIRGIN)
     ...
     public int purge() {
         ...
         synchronized(queue) {
             for (int i = queue.size(); i > 0; i--) {
                 if (queue.get(i).state == java.util.TimerTask.CANCELLED) {
                     queue.quickRemove(i);
                     result++;
                 }
             }
...
class TaskQueue {
     ...
     */
    private java.util.TimerTask[] queue = new java.util.TimerTask[128];
     ...
     */
    void add(java.util.TimerTask task) {
    ...
    private void fixUp(int k) {
        while (k > 1) {
            ...
            if (queue[j].nextExecutionTime <= queue[k].nextExecutionTime)
                break;
            java.util.TimerTask tmp = queue[j];  queue[j] = queue[k]; queue[k] = tmp;
            k = j;
        }
    }
