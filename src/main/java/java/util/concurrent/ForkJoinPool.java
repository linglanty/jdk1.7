...
public class ForkJoinPool extends java.util.concurrent.AbstractExecutorService {
    ...
    /**
     * Factory for creating new {@link java.util.concurrent.ForkJoinWorkerThread}s.
     ...
     * @param w the worker
     */
    final void work(java.util.concurrent.ForkJoinWorkerThread w) {
    ...
    final void signalWork() {
        ...
        while ((((e = (int)(c = ctl)) | (u = (int)(c >>> 32))) &
            ...
            if (e > 0) {                         // release a waiting worker
                int i; java.util.concurrent.ForkJoinWorkerThread w; java.util.concurrent.ForkJoinWorkerThread[] ws;
                if ((ws = workers) == null ||
                    (i = ~e & SMASK) >= ws.length ||
                    (w = ws[i]) == null)
                    break;
    ...
    private boolean tryReleaseWaiter() {
        long c; int e, i; java.util.concurrent.ForkJoinWorkerThread w; java.util.concurrent.ForkJoinWorkerThread[] ws;
        if ((e = (int)(c = ctl)) > 0 &&
            (int)(c >> AC_SHIFT) < 0 &&
            (ws = workers) != null &&
            (i = ~e & SMASK) < ws.length &&
     ...
     */
    private boolean scan(java.util.concurrent.ForkJoinWorkerThread w, int a) {
        int g = scanGuard; // mask 0 avoids useless scans if only one active
        int m = (parallelism == 1 - a && blockedCount == 0) ? 0 : g & SMASK;
        java.util.concurrent.ForkJoinWorkerThread[] ws = workers;
        if (ws == null || ws.length <= m)         // staleness check
            return false;
        for (int r = w.seed, k = r, j = -(m + m); j <= m + m; ++j) {
            java.util.concurrent.ForkJoinTask<?> t; java.util.concurrent.ForkJoinTask<?>[] q; int b, i;
            java.util.concurrent.ForkJoinWorkerThread v = ws[k & m];
     ...
     */
    private boolean tryAwaitWork(java.util.concurrent.ForkJoinWorkerThread w, long c) {
        int v = w.eventCount;
        ...
        for (boolean rescanned = false;;) {
            ...
            if (!rescanned) {
                ...
                if (ws != null && m < ws.length) {
                    rescanned = true;
                    for (int i = 0; i <= m; ++i) {
                        java.util.concurrent.ForkJoinWorkerThread u = ws[i];
                        if (u != null) {
     ...
     */
    private void idleAwaitWork(java.util.concurrent.ForkJoinWorkerThread w, long currentCtl,
                               long prevCtl, int v) {
        if (w.eventCount == v) {
            if (shutdown)
                tryTerminate(false);
            java.util.concurrent.ForkJoinTask.helpExpungeStaleExceptions(); // help clean weak refs
     ...
     * @param t the task
     */
    private void addSubmission(java.util.concurrent.ForkJoinTask<?> t) {
        final ReentrantLock lock = this.submissionLock;
        lock.lock();
        try {
            java.util.concurrent.ForkJoinTask<?>[] q; int s, m;
    ...
    private void growSubmissionQueue() {
        java.util.concurrent.ForkJoinTask<?>[] oldQ = submissionQueue;
        int size = oldQ != null ? oldQ.length << 1 : INITIAL_QUEUE_CAPACITY;
        if (size > MAXIMUM_QUEUE_CAPACITY)
            throw new java.util.concurrent.RejectedExecutionException("Queue capacity exceeded");
        if (size < INITIAL_QUEUE_CAPACITY)
            size = INITIAL_QUEUE_CAPACITY;
        java.util.concurrent.ForkJoinTask<?>[] q = submissionQueue = new java.util.concurrent.ForkJoinTask<?>[size];
        int mask = size - 1;
        int top = queueTop;
        int oldMask;
    ...
    private boolean tryPreBlock() {
        ...
        if (UNSAFE.compareAndSwapInt(this, blockedCountOffset, b, b + 1)) {
            int pc = parallelism;
            do {
                java.util.concurrent.ForkJoinWorkerThread[] ws; java.util.concurrent.ForkJoinWorkerThread w;
                int e, ac, tc, rc, i;
                long c = ctl;
                int u = (int)(c >>> 32);
                if ((e = (int)c) < 0) {
     ...
     */
    final void tryAwaitJoin(java.util.concurrent.ForkJoinTask<?> joinMe) {
        int s;
     ...
     */
    final void timedAwaitJoin(java.util.concurrent.ForkJoinTask<?> joinMe, long nanos) {
    ...
    private void addWorker() {
        Throwable ex = null;
        java.util.concurrent.ForkJoinWorkerThread t = null;
        try {
        ...
        if (t == null) {  // null or exceptional factory return
            ...
            if (!tryTerminate(false) && ex != null &&
                !(Thread.currentThread() instanceof java.util.concurrent.ForkJoinWorkerThread))
     ...
     */
    final int registerWorker(java.util.concurrent.ForkJoinWorkerThread w) {
        /*
        ...
        for (int g;;) {
            ...
            if (((g = scanGuard) & SG_UNIT) == 0 &&
                ...
                for (java.util.concurrent.ForkJoinWorkerThread u : ws) {
     ...
     * @param w the worker
     */
    final void deregisterWorker(java.util.concurrent.ForkJoinWorkerThread w, Throwable ex) {
        int idx = w.poolIndex;
        int sc = w.stealCount;
        int steps = 0;
    ...
    private void startTerminating() {
        ...
        for (int pass = 0; pass < 3; ++pass) {
            java.util.concurrent.ForkJoinWorkerThread[] ws = workers;
            if (ws != null) {
                for (java.util.concurrent.ForkJoinWorkerThread w : ws) {
                    if (w != null) {
                        w.terminate = true;
                        if (pass > 0) {
                            w.cancelTasks();
    ...
    private void cancelSubmissions() {
        while (queueBase != queueTop) {
            java.util.concurrent.ForkJoinTask<?> task = pollSubmission();
            if (task != null) {
                try {
                    task.cancel(false);
    ...
    private void terminateWaiters() {
        java.util.concurrent.ForkJoinWorkerThread[] ws = workers;
        if (ws != null) {
            java.util.concurrent.ForkJoinWorkerThread w; long c; int i, e;
            int n = ws.length;
    ...
    public ForkJoinPool(int parallelism,
        ...
        this.submissionQueue = new java.util.concurrent.ForkJoinTask<?>[INITIAL_QUEUE_CAPACITY];
        ...
        workers = new java.util.concurrent.ForkJoinWorkerThread[n + 1];
     ...
     */
    public <T> T invoke(java.util.concurrent.ForkJoinTask<T> task) {
        ...
        if ((t instanceof java.util.concurrent.ForkJoinWorkerThread) &&
            ((java.util.concurrent.ForkJoinWorkerThread)t).pool == this)
     ...
     */
    private <T> void forkOrSubmit(java.util.concurrent.ForkJoinTask<T> task) {
        java.util.concurrent.ForkJoinWorkerThread w;
        Thread t = Thread.currentThread();
        if (shutdown)
            throw new java.util.concurrent.RejectedExecutionException();
        if ((t instanceof java.util.concurrent.ForkJoinWorkerThread) &&
            (w = (java.util.concurrent.ForkJoinWorkerThread)t).pool == this)
            w.pushTask(task);
        else
            addSubmission(task);
    }
    ...
    public void execute(Runnable task) {
        ...
        if (task instanceof java.util.concurrent.ForkJoinTask<?>) // avoid re-wrap
            job = (java.util.concurrent.ForkJoinTask<?>) task;
        else
            job = java.util.concurrent.ForkJoinTask.adapt(task, null);
        forkOrSubmit(job);
    }
     ...
     *         scheduled for execution
     */
    public <T> java.util.concurrent.ForkJoinTask<T> submit(java.util.concurrent.ForkJoinTask<T> task) {
        if (task == null)
            throw new NullPointerException();
        forkOrSubmit(task);
        return task;
     ...
     *         scheduled for execution
     */
    public <T> java.util.concurrent.ForkJoinTask<T> submit(java.util.concurrent.Callable<T> task) {
        if (task == null)
            throw new NullPointerException();
        java.util.concurrent.ForkJoinTask<T> job = java.util.concurrent.ForkJoinTask.adapt(task);
        forkOrSubmit(job);
        return job;
    }
     ...
     *         scheduled for execution
     */
    public <T> java.util.concurrent.ForkJoinTask<T> submit(Runnable task, T result) {
        if (task == null)
            throw new NullPointerException();
        java.util.concurrent.ForkJoinTask<T> job = java.util.concurrent.ForkJoinTask.adapt(task, result);
        forkOrSubmit(job);
        return job;
    }
     ...
     *         scheduled for execution
     */
    public java.util.concurrent.ForkJoinTask<?> submit(Runnable task) {
        if (task == null)
            throw new NullPointerException();
        java.util.concurrent.ForkJoinTask<?> job;
        if (task instanceof java.util.concurrent.ForkJoinTask<?>) // avoid re-wrap
    ...
    public <T> List<java.util.concurrent.Future<T>> invokeAll(Collection<? extends java.util.concurrent.Callable<T>> tasks) {
        ArrayList<java.util.concurrent.ForkJoinTask<T>> forkJoinTasks =
            new ArrayList<java.util.concurrent.ForkJoinTask<T>>(tasks.size());
        for (java.util.concurrent.Callable<T> task : tasks)
            forkJoinTasks.add(java.util.concurrent.ForkJoinTask.adapt(task));
        invoke(new InvokeAll<T>(forkJoinTasks));

        @SuppressWarnings({"unchecked", "rawtypes"})
            List<java.util.concurrent.Future<T>> futures = (List<java.util.concurrent.Future<T>>) (List) forkJoinTasks;
        return futures;
    }

    static final class InvokeAll<T> extends RecursiveAction {
        final ArrayList<java.util.concurrent.ForkJoinTask<T>> tasks;
        InvokeAll(ArrayList<java.util.concurrent.ForkJoinTask<T>> tasks) { this.tasks = tasks; }
        public void compute() {
            try { invokeAll(tasks); }
    ...
    protected int drainTasksTo(Collection<? super java.util.concurrent.ForkJoinTask<?>> c) {
        int count = 0;
        while (queueBase != queueTop) {
            java.util.concurrent.ForkJoinTask<?> t = pollSubmission();
            if (t != null) {
                c.add(t);
                ++count;
            }
     ...
     * is a {@link java.util.concurrent.ForkJoinWorkerThread}, this method possibly
     * arranges for a spare thread to be activated if necessary to
     * ensure sufficient parallelism while the current thread is blocked.
     *
     * <p>If the caller is not a {@link java.util.concurrent.ForkJoinTask}, this method is
    ...
    public static void managedBlock(ManagedBlocker blocker)
        ...
        Thread t = Thread.currentThread();
        if (t instanceof java.util.concurrent.ForkJoinWorkerThread) {
            java.util.concurrent.ForkJoinWorkerThread w = (java.util.concurrent.ForkJoinWorkerThread) t;
    ...
    protected <T> java.util.concurrent.RunnableFuture<T> newTaskFor(Runnable runnable, T value) {
        return (java.util.concurrent.RunnableFuture<T>) java.util.concurrent.ForkJoinTask.adapt(runnable, value);
    }

    protected <T> java.util.concurrent.RunnableFuture<T> newTaskFor(java.util.concurrent.Callable<T> callable) {
        return (java.util.concurrent.RunnableFuture<T>) java.util.concurrent.ForkJoinTask.adapt(callable);
    }
    ...
    static {
        ...
        try {
            ...
            Class a = java.util.concurrent.ForkJoinTask[].class;
