...
public class FutureTask<V> implements java.util.concurrent.RunnableFuture<V> {
    ...
    private V report(int s) throws java.util.concurrent.ExecutionException {
        ...
        throw new java.util.concurrent.ExecutionException((Throwable)x);
    }
    ...
    public FutureTask(Runnable runnable, V result) {
        this.callable = java.util.concurrent.Executors.callable(runnable, result);
        this.state = NEW;       // ensure visibility of callable
    }
    ...
    public void run() {
        if (state != NEW ||
            ...
            return;
        try {
            java.util.concurrent.Callable<V> c = callable;
            if (c != null && state == NEW) {
                V result;
                boolean ran;
                try {
